function [V, D] = findShapeVariations3d(z, P, TriangleIndex)

	X = zeros(size(P, 1) * size(P, 2), size(P, 3));
	reshape_size = size(z);

	for i = 1:size(P, 3)
		Pi = P(:, :, i);
		x = Pi - z;
		X(:, i) = x(:);
	end

	C = (X*X')/size(P, 3);
	[V, D] = eig(C);
	[V, D] = sortEigenVectors(V, D);
	for i = 1:3
		g1 = z + reshape(2*sqrt(D(i))*V(:, i), reshape_size);
		g2 = z - reshape(2*sqrt(D(i))*V(:, i), reshape_size);
		figure;
		subplot(1, 3, 1);
		patch('Vertices', g1', 'Faces', TriangleIndex, 'FaceColor', 'green', 'Edgecolor', 'red');
		view(3);
		axis equal vis3d;
		subplot(1, 3, 2);
		patch('Vertices', z', 'Faces', TriangleIndex, 'FaceColor', 'green', 'Edgecolor', 'red');
		view(3);
		axis equal vis3d;
		subplot(1, 3, 3);
		patch('Vertices', g2', 'Faces', TriangleIndex, 'FaceColor', 'green', 'Edgecolor', 'red');
		view(3);
		axis equal vis3d;
	end
end